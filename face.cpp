...
int Face::detect(const cv::Mat& bgr, std::vector<Object>& objects, int target_size, float prob_threshold, float nms_threshold)
{
    int img_w = bgr.cols;
    int img_h = bgr.rows;
...
void Face::draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        cv::circle(bgr, obj.pts[0], 2, cv::Scalar(0, 0, 255), -1);
        cv::circle(bgr, obj.pts[1], 2, cv::Scalar(0, 255, 0), -1);
        cv::circle(bgr, obj.pts[2], 2, cv::Scalar(255, 0, 0), -1);
        cv::circle(bgr, obj.pts[3], 2, cv::Scalar(0, 255, 255), -1);
        cv::circle(bgr, obj.pts[4], 2, cv::Scalar(255, 255, 0), -1);
        cv::rectangle(bgr, obj.rect, cv::Scalar(255, 255, 0),2);
